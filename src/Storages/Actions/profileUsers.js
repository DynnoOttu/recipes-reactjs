...
export const getProfile = (navigate) => async (dispatch) => {
    try {
        ...
        const result = await axios.get(`${process.env.REACT_APP_BASE_URL}/users/users`, headers)
...
export const editProfile = (data, navigate) => async (dispatch) => {
    try {
        ...
        const result = await axios.put(`${process.env.REACT_APP_BASE_URL}/users/users`, data, headers)
